...
class CodeFile(Base):
    ...
    project_root = Column(Text, nullable=False, index=True) # Root directory of the indexed project
    ...
    __table_args__ = (
        Index('ix_code_files_project_root_file_path', 'project_root', 'file_path', unique=True),
    )

    def __repr__(self):
        return f"<CodeFile(id={self.id}, file_name='{self.file_name}', project_root='{self.project_root}')>"
